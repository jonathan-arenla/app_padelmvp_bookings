...
statsRouter.get("/", requireAuth, async (req, res) => {
  ...
  try {
    ...
    const userPairs = await prisma.tournamentPair.findMany({
      where: {
    ...
    const partnerCounts: Record<string, { count: number; name: string; avatarUrl: string | null }> = {};
    const opponentCounts: Record<string, { count: number; name: string; avatarUrl: string | null }> = {};
    ...
    const history = matches.map((m) => {
      ...
      if (myPair) {
        ...
        if (partner) {
          if (!partnerCounts[partner.id]) partnerCounts[partner.id] = { count: 0, name: partner.name, avatarUrl: partner.avatarUrl };
      ...
      if (!won && opponentPair) {
        [opponentPair.player1, opponentPair.player2].forEach(opp => {
          if (opp) {
            if (!opponentCounts[opp.id]) opponentCounts[opp.id] = { count: 0, name: opp.name, avatarUrl: opp.avatarUrl };
